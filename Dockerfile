...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/commit_vibe ./

USER nobody

# If using an environment that doesn't automatically reap zombie processes
# like combos of `tini` and `dumb-init`, set a CMD like this:
# CMD ["sh", "-c", "PHX_SERVER=true /app/bin/commit_vibe start"]
CMD ["sh", "-c", "PHX_SERVER=true /app/bin/commit_vibe start"]
