...
impl GameState {
    ...
    pub fn new_survival(commands: Vec<Command>, total_rounds: usize) -> Self {
    ...
    /// Handle survival mode char input — any error = game over
    pub fn survival_handle_char(&mut self, ch: char) {
        let expected_pos = self.typing.cursor_pos;
        self.typing.handle_char(ch);

        // Check if the char we just typed was wrong
        if self.typing.errors.contains(&expected_pos) {
            // Game over - record partial result
            ...
            self.survival_finish();
            return;
        }

        if self.typing.is_complete() {
            self.survival_complete_round();
        }
    }

    fn survival_complete_round(&mut self) {
        let elapsed = self.round_start.elapsed();
        let result = RoundResult {
            command: self.commands[self.current_round].text.clone(),
            elapsed,
